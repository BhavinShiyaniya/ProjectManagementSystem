...
def teamleader_required(function =None, redirect_field_name = REDIRECT_FIELD_NAME, login_url='login'):
    # actual decorator user defined function
    actual_decorator = user_passes_test(lambda user:user.is_active and user.is_teamleader, login_url=login_url, redirect_field_name=redirect_field_name)
    if function:
        return actual_decorator(function)
    return actual_decorator

def tester_required(function =None, redirect_field_name = REDIRECT_FIELD_NAME, login_url='login'):
    # actual decorator user defined function
    actual_decorator = user_passes_test(lambda user:user.is_active and user.is_tester, login_url=login_url, redirect_field_name=redirect_field_name)
    if function:
        return actual_decorator(function)
    return actual_decorator